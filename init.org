...
To see whether a file contains a flash card, we check whether the org buffer has the property =ANKI_NOTE_TYPE=, the only required property for a flash card.
#+begin_src emacs-lisp
