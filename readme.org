...
* Meta
Currently using Emacs-Plus 28 from [[https://github.com/d12frosted/homebrew-emacs-plus][here]] compiled with =--with-native-comp= and =--with-xwidgets=. Compile time was quite long on 2.9Ghz 2017 MBP. Looking forward to testing on M1 Pro.
...
(add-to-list 'xenops-math-latex-process-alist
	       ...
	       :image-converter ("dvisvgm --pdf %f -n -b %B -c %S -o %O")
	       ))

(setq xenops-math-latex-process 'my/pdfsvgm)
