...
Currently using Emacs-Plus 28 from [[https://github.com/d12frosted/homebrew-emacs-plus][here]] compiled with =--with-native-comp=, =--with-imagemagick= and =--with-xwidgets=. Compile time was cca 30min on 2.9Ghz 2017 MBP. Looking forward to testing on M1 Pro. Took about 10min on M1 Pro. Wew.
...
** Time tracking
If inactive for 5 minutes, ask what to do with the time. On macOS it considers actual user idle time, not just emacs, so keeping it low.
#+begin_src emacs-lisp
...
#+end_src
** Not deft
Faster than Deft, still not that fast though, might as well keep it around though.
#+begin_src emacs-lisp
